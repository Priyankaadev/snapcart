...
import {  LogOut, Package, Search, ShoppingCartIcon, SidebarCloseIcon, User, XIcon } from 'lucide-react'
...
const Nav = ({user}:{user:IUser}) => {
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl
        ...
        <div className='flex items-center gap-3 md:gap-6 relative'>
          <div className='bg-white rounded-full w-11 h-11 flex items-center justify-center
           shadow-md hover:scale-105 transition md:hidden' onClick={()=>setSearchBarOpen((prev)=>!prev)}>
            ...
            <Link href={''} className='relative bg-white rounded-full w-11 h-11 flex items-center justify-center
            ...
            overflow-hidden hover:scale-105 transition-transform ' onClick={()=>setOpen(prev=>!prev)}>
           ...
           <AnimatePresence>
            ...
            >
                ...
                <div className='w-10 h-10 rounded-full relative bg-green-100 flex items-center justify-center overflow-hidden'>
                    {user.image? <Image src={user.image} alt='user' width={50} height={50} className="object-cover rounded-full" />:<User/>}
                </div>
                <div>
                ...
                <Link href={""} className='flex items-center gap-2 px-3 py-3 hover:bg-green-50 rounded-lgtext-gray-800
                font-medium' onClick={()=>(setOpen(false))}>
                <Package className='w-5 h-5 text-green-600' />
                My Orders
                </Link>
                <button className='flex items-center gap-2 w-full text-left px-3 py-3 hover:bg-red-50 rounded-lg text-gray-700
                font-medium' onClick={()=>{
                    setOpen(false)
          ...
          <AnimatePresence>
            ...
            shadow-lg z-40 flex items-center px-4 py-2'>
               ...
               <button onClick={()=>setSearchBarOpen((prev)=>!prev)}><XIcon className='text-gray-500 w-5 h-5'/></button>
                </motion.div>
                }
          </AnimatePresence>
            </div> 
